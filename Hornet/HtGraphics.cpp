...
const Colour HtGraphics::SKYBLUE = Colour(135, 135, 206, 235);
...
Result HtGraphics::DrawPointList(Vector2D points[], Colour colour, const int numPoints)
{
    ...
    {
        points[i] = HtCamera::instance.NativeTransform(points[i]);
        pList[i].x = int(points[i].XValue);
        pList[i].y = int(points[i].YValue);
    }
...
Result HtGraphics::WriteTextCentered(Vector2D centre, std::string text, Colour colour, FontIndex font, double angle, double scale)
{
    ...
    if (text.size() > 0)
    {
        ...
        {
            SDL_Rect targetArea;
            targetArea.x = int(centre.XValue - texture.height * scale/2);
            targetArea.y = int(centre.YValue- texture.width * scale/2);
...
{	
    ...
    m_pictureMap.clear();
    m_filenameMap.clear();
    m_nextPictureIndex = 0;
}

void HtGraphics::ReleaseAllFonts()
{
    // Start at the beginning
    std::map<PictureIndex, TTF_Font*>::iterator picit = m_fontList.begin();
